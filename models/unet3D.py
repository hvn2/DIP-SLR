...
class UNet(nn.Module):
    ...
    def __init__(self, num_input_channels=3, num_output_channels=3, 
                       ...
                       upsample_mode='deconv', pad='zero', norm_layer=nn.InstanceNorm3d, need_sigmoid=True, need_bias=True):
...
class unetUp(nn.Module):
    def __init__(self, out_size, upsample_mode, need_bias, pad, same_num_filt=False):
        ...
        if upsample_mode == 'deconv':
            self.up= nn.ConvTranspose3d(num_filt, out_size, 4, stride=2, padding=1)
            self.conv= unetConv2(out_size * 2, out_size, None, need_bias, pad)
        elif upsample_mode=='trilinear' or upsample_mode=='nearest':
    ...
    def forward(self, inputs1, inputs2):
        ...
        if (inputs2.size(4) != in1_up.size(4)) or (inputs2.size(3) != in1_up.size(3)):
            diff2 = (inputs2.size(3) - in1_up.size(3)) // 2 
            diff3 = (inputs2.size(4) - in1_up.size(4)) // 2 
            inputs2_ = inputs2[:, :, :, diff2 : diff2 + in1_up.size(3), diff3 : diff3 + in1_up.size(4)]
        else:
            inputs2_ = inputs2
